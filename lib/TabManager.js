var TabManager = React.createFactory(React.createClass({
	...
	addWindow: function () {
		...
		if (tabs.length == 1) {
			...
			browser.windows.update(tab.windowId, { active: true });
		}
		else if (tabs.length != 0) {
			const tabIds = tabs.map(t => t.id);
			// const first = tabIds[0];
			browser.tabs.move(tabIds, { windowId: tabs[0].windowId, index: -1 })
			// browser.windows.create({ tabId: first, state: "minimized" })
			// 	.then(
			// 		newWindow =>
			...
			// 					ok => {
			// 						alert(ok);
			// 						// browser.windows.update(newWindow.id, { focused: true });
			// 					},
			// 					error => alert(error)
			// 				),
			// 		error => alert(error)
		...
		if (tabs.length) {
			...
			chrome.windows.getCurrent(function (w) {
				browser.tabs.query({ active: true }).then(tabs => {
					tabs.map(t => browser.tabs.update(t.id, { pinned: !t.pinned }))
				});
				// chrome.tabs.getSelected(w.id, function (t) {
